...
### 钱包奖励（零花钱激励）
- 中译英 / 单词练习**拿满分**：随机奖励整元零花钱（默认 1～3 元），每道题（或每个单词的某个阶段）**历史上只奖一次**。
- 单题**首次**得分低于 60：随机扣除整元零花钱（默认 1～2 元），只在首次有效提交时扣，复习不会扣，AI 批改失败的提交不奖不罚。
...
- **钱包管理**：在后台「钱包」页设置奖励/扣除金额区间与提现门槛（单位元），处理待发放的提现（标记「已发放」），手动 ±调整余额（需填原因），查看分页流水。
...
**钱包（`wallet_transactions`）** 一张带符号金额的流水表（`reward`/`penalty`/`withdraw`/`adjust`），余额由 `SUM(amount_cents)` **推导**、从不落库。两个提交路由批改后各调一次结算并把奖惩附在响应里；满分每个 ref 只奖一次，低分仅首次有效提交扣一次，AI 失败的提交不结算。余额可为负（设计如此）。规则有 `tests/wallet-rules.test.mjs` 覆盖。
