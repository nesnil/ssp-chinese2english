...
### 高考词汇练习
- 基于高考考纲词汇（`senior-candidate` 分类）的**A–Z 分组、10 词一关**闯关表，与中考词汇练习并列于首页标签。
...
- **单词管理**：列出全部单词，按**分类**（中考考纲/高考考纲词汇/四六级候选等）筛选、**A–Z 首字母**筛选、搜索、分页和定位单词；增删改查，编辑词性（下拉）、释义、例句、分类（多选），列表内可试听发音，缺发音时可生成/重新生成 mp3。
...
- **模型历史**：后台「模型」页提供「模型对话历史」和「Siri 对话历史」两个视图，按批改 / TTS / Siri、成功 / 失败筛选模型调用记录，支持搜索和查看请求/响应摘要，便于排查批改、发音生成和语音钱包解析问题；记录存入 SQLite（`model_interactions` 表），不保存 API Key。
...
> 注意：`npm run dev` **不是** Vite 热更新开发服务器；它会先生成题库和客户端静态文件，再用 `tsx watch` 跑服务端并由 Express 托管 `dist/public`。改了客户端代码后重新执行 `npm run dev` 或 `npm run build:client` 才能在浏览器看到。

### 常用命令（均在 `web/` 下）

```bash
npm run dev          # 构建种子 + 构建客户端 + tsx watch 服务端
